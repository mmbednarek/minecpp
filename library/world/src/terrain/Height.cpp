...
{
   double result = 70.0;
   math::Vector2 perlin_pos{static_cast<double>(x), static_cast<double>(z)};

   result += base_perlin.at(perlin_pos / 300.0) * 64;
   result += perlin_detail0.at(perlin_pos / 200.0 + math::Vector2{0.1, 0.3}) * 180;
   ...
   if (result > 90) {
      result += perlin_detail1.at(perlin_pos / 12.0 + math::Vector2{0.6, 0.32}) * 40;
   } else {
      result += perlin_detail1.at(perlin_pos / 45.0 + math::Vector2{0.6, 0.32}) * 30;
   }
