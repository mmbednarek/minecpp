...
void Repository::serialize_no_header(minecpp::nbt::Writer &w) const
{
   ...
   std::for_each(block_states.begin(), block_states.end(),
   ...
   std::for_each(items.begin(), items.end(), [&w](const auto &value) { value.serialize_no_header(w); });
   ...
   std::for_each(bool_properties.begin(), bool_properties.end(),
                 [&w](const auto &value) { value.serialize_no_header(w); });
   w.end_compound();
}
            ...
            case minecpp::nbt::TagId::Compound: {
               if (in_field_name == "BlockStates") {
                  ...
                  std::generate(ls.begin(), ls.end(),
                                [&r]() { return BlockStateEntry::deserialize_no_header(r); });
               ...
               if (in_field_name == "Items") {
                  ...
                  std::generate(ls.begin(), ls.end(), [&r]() { return ItemEntry::deserialize_no_header(r); });
                  res.set_property(in_field_name, ls);
                  return;
               }
               if (in_field_name == "EnumProperties") {
                  ...
                  std::generate(ls.begin(), ls.end(),
                                [&r]() { return EnumPropertyEntry::deserialize_no_header(r); });
                  res.set_property(in_field_name, ls);
                  return;
               }
               if (in_field_name == "IntProperties") {
                  ...
                  std::generate(ls.begin(), ls.end(),
                                [&r]() { return IntPropertyEntry::deserialize_no_header(r); });
                  res.set_property(in_field_name, ls);
                  return;
               }
               if (in_field_name == "BoolProperties") {
                  ...
                  std::generate(ls.begin(), ls.end(),
                                [&r]() { return BoolPropertyEntry::deserialize_no_header(r); });
                  res.set_property(in_field_name, ls);
                  return;
               }
               break;
