    ...
    m_component(structure.package, m_header_name)
{
   ...
   std::for_each(structure.messages.begin(), structure.messages.end(), [this](const Semantics::Message &msg) {
      ...
      message_class.add_public(static_method(
              msg.name, "deserialize_no_header",
              {
...
void put_deserialize_logic(const Semantics::Message &msg, mb::codegen::statement::collector &col)
{
   ...
   std::for_each(
           ...
           [&tag_switch](const std::pair<const Semantics::TypeVariant, std::any> &pair) {
                                ...
                                mb::codegen::statement::collector &col, mb::size depth)
{
   ...
   col << if_statement(raw("list_info{}.size > 0", depth), [&deserializer, depth](statement::collector &col) {
      ...
      std::for_each(
              deserializer.elems.begin(), deserializer.elems.end(),
              [depth, &list_type_switch](const std::pair<const Semantics::TypeVariant, std::any> &pair) {
                 if (!pair.second.has_value()) {
                    return;
                 }
                 ...
                 if (pair.second.type() == typeid(Semantics::ListDeserializer)) {
                    ...
                    list_type_switch.add(raw("minecpp::nbt::TagId::List"),
                                         [&elem_deserializer, depth](statement::collector &col) {
...
void put_serialize_logic(mb::codegen::statement::collector &col, Semantics::Type type,
                         const mb::codegen::expression &value)
{
   using namespace mb::codegen;

   if (type.m_repeated > 0) {
      col << call("w.begin_list_no_header",
                  raw(type.m_repeated == 1 ? variant_to_nbt_tag(type.variant) : "minecpp::nbt::TagId::List"),
                  method_call(value, "size"));
      col << call("std::for_each", method_call(value, "begin"), method_call(value, "end"),
                  lambda(
                          {
                                  {"const auto", "&value"}
      },
   ...
   case Semantics::TypeVariant::String: col << call("w.write_string_content", value); break;
   case Semantics::TypeVariant::Float: col << call("w.write_float_content", value); break;
   case Semantics::TypeVariant::Double: col << call("w.write_double_content", value); break;
   case Semantics::TypeVariant::Bytes: col << call("w.write_bytes_content", value); break;
   case Semantics::TypeVariant::Ints: col << call("w.write_ints_content", value); break;
   case Semantics::TypeVariant::Longs: col << call("w.write_longs_content", value); break;
   ...
   case Semantics::TypeVariant::Map:
      ...
      break;
   default: col << method_call(value, "serialize_no_header", raw("w"));
   }
}
