...
void Properties::serialize(::minecpp::network::message::Writer &writer) const {
   ...
   writer.write_varint(static_cast<std::uint32_t>(this->meta.size()));
...
Properties Properties::deserialize(::minecpp::network::message::Reader &reader) {
   ...
   const auto map_size = reader.read_varint();
   std::generate_n(std::inserter(result.meta, result.meta.begin()), static_cast<std::size_t>(map_size), [&reader]() {
