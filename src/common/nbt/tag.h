...
template<TagID t>
inline auto tagid_default() = delete;
template<typename T>
constexpr TagID tagid_from_type() {
...
#define tagid_default_of(tagid, ...) \
	...
	template <> constexpr TagID tagid_from_type<tagid_type(tagid)>() { return tagid; };
...
tagid_default_of(TagID::Byte, static_cast<uint8_t>(0))
tagid_default_of(TagID::Short, static_cast<short>(0))
tagid_default_of(TagID::Int, static_cast<int>(0))
...
template<typename P>
class PayloadTag : public Tag {
public:
    PayloadTag(std::string& name, P payload) :
		Tag(tagid_from_type<P>(), name),
...
template<typename T>
std::vector<T> &ListPayload::value() {
	if(tagid_from_type<T>() != _type)
