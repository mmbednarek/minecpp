...
#include "../common/nbt/reader.h"
#include "../common/nbt/compound_map.h"
...
TEST(NBT_Parser, TagFloat) {
	char buff[] {0x05, 0x00, 0x10, 'A', 'b', 's', 'o', 'r', 'p', 't', 'i', 'o', 'n', 'A', 'm', 'o', 'u', 'n', 't', 0x40, (char)0xa0, 0x00, 0x00};
	auto stream = std::stringstream();
	stream.write(buff, sizeof(buff));
...
TEST(NBT_Parser, TagDouble) {
	char buff[] {0x06, 0x00, 0x11, 0x42, 0x6f, 0x72, 0x64, 0x65, 0x72, 0x57, 0x61, 0x72, 0x6e, 0x69, 0x6e, 0x67, 0x54, 0x69, 0x6d, 0x65, 0x40, 0x2e, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
	auto stream = std::stringstream();
	stream.write(buff, sizeof(buff));
...
TEST(NBT_Parser, MultipleTags) {
	char buff[] {0x03, 0x00, 0x06, 'T', 'e', 's', 't', 's', '1', 0x10, 0x21, 0x32, 0x43, 0x02, 0x00, 0x03, 'a', 'b', 'c', 0x10, 0x20};
	auto stream = std::stringstream();
	stream.write(buff, sizeof(buff));
...
TEST(NBT_Parser, StringListTag) {
	char buff[] {0x09, 0x00, 0x0c, 0x53, 0x65, 0x72, 0x76,
			  	 0x65, 0x72, 0x42, 0x72, 0x61, 0x6e, 0x64,
			  	 0x73, 0x08, 0x00, 0x00, 0x00, 0x02, 0x00,
			  	 0x07, 0x76, 0x61, 0x6e, 0x69, 0x6c, 0x6c, 0x61, 0x00, 0x07, 'm', 'i', 'n', 'e', 'c', 'p', 'p'};
	auto stream = std::stringstream();
	stream.write(buff, sizeof(buff));
	Parser r(stream);
...
TEST(NBT_Parser, CompoundTag) {
	char buff[] {0x0a, 0x00, 0x04, 'D', 'a', 't', 'a',
			  	 0x02, 0x00, 0x03, 'N', 'u', 'm', 0x01, 0x23,
			  	 0x03, 0x00, 0x05, 'S', 't', 'u', 'f', 'f', 0x10, 0x20, 0x30, 0x40,
			  	 0x00};
	auto stream = std::stringstream();
	stream.write(buff, sizeof(buff));
	Parser r(stream);
...
TEST(NBT_Parser, IntArrayTag) {
	char buff[] {0x0b, 0x00, 0x06, 'i', 'n', 't', 'a', 'r', 'r', 0x00, 0x00, 0x00, 0x02,
...
TEST(NBT_Parser, LongArrayTag) {
	char buff[] {0x0c, 0x00, 0x07, 'l', 'o', 'n', 'g', 'a', 'r', 'r', 0x00, 0x00, 0x00, 0x02,
				 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x10, 0x20, 0x30, 0x40, 0x50, 0x60, 0x70, (char)0x80};
	auto stream = std::stringstream();
	stream.write(buff, sizeof(buff));
	Parser r(stream);
	...
	ASSERT_EQ(as_string(tag), "[L;\n\t72623859790382856,\n\t1161981756646125696\n]");
}

TEST(NBT_Compound, Map) {
	char buff[] {0x02, 0x00, 0x05, 's', 't', 'u', 'f', 'f', 0x12, 0x34,
			     0x02, 0x00, 0x04, 't', 'e', 's', 't',  0x43, 0x21,
			     0x02, 0x00, 0x06, 'a', 'b', 'c', '1', '2', '3', 0x11, 0x22,
			     0x00};
	auto stream = std::stringstream();
	stream.write(buff, sizeof(buff));
	Reader r(stream);
...
TEST(NBT_Reader, Int) {
	char buff[] {0x0a, 0x00, 0x00, 0x03, 0x00, 0x06, 'T', 'e', 's', 't', 's', '1', 0x10, 0x21, 0x32, 0x43, 0x00};
	auto stream = std::stringstream();
	stream.write(buff, sizeof(buff));
...
TEST(NBT_Reader, Compound) {
	char buff[] {0x0a, 0x00, 0x00,
			  	 	0x03, 0x00, 0x06, 'T', 'e', 's', 't', 's', '1', 0x12, 0x34, 0x56, 0x78,
			      	0x0a, 0x00, 0x06, 'V', 'a', 'l', 'u', 'e', 's',
						 0x02, 0x00, 0x03, 'F', 'o', 'o', 0x12, 0x35,
				 	0x00,
				 0x00};
	auto stream = std::stringstream();
	stream.write(buff, sizeof(buff));
