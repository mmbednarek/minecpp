...
{
public:
   void on_reply_empty_chunk(Context & /*a*/,
                             const minecpp::net::storage::cb::ReplyEmptyChunk &message) override
   {
      spdlog::info("reply: empty chunk {} {}", message.position.x(), message.position.y());
   }

   void on_reply_chunk(Context & /*a*/, const minecpp::net::storage::cb::ReplyChunk &message) override
   {
      Chunk chunk;
   ...
   void on_failure(Context & /*a*/, std::uint8_t msg_code) override
   {
