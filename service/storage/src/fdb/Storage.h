...
{
 public:
   Storage(std::future<void> network_thread, FDBDatabase *database);
   ~Storage() override;
   ...
   bool update_chunk(game::ChunkPosition position, const std::function<void(proto_chunk::Chunk &chunk)>& callback) override;
   bool add_chunk_subscription(game::ChunkPosition position, const proto_storage::ClientId &client_id) override;
 private:

   Transaction create_transaction();

   std::future<void> m_network_thread;
