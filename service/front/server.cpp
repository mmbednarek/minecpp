...
Protocol::Handler &Server::get_handler(const Protocol::State state) { return *m_handlers[state]; }

void Server::index_connection(boost::uuids::uuid index, std::size_t id) { conn_ids[index] = id; }

bool Server::has_connection(player::Id player_id) { return conn_ids.contains(player_id); }
