...
Config get_config(const std::string &file_name)
{
   ...
   response.encryption_enabled     = config["encryption"]["enabled"].as<bool>(false);
   response.encryption_private_key = config["encryption"]["private_key"].as<std::string>("");
   response.encryption_pass_phrase = config["encryption"]["pass_phrase"].as<std::string>("");
