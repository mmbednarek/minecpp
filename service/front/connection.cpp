...
{
   boost::asio::async_write(m_socket, boost::asio::buffer(buff, size),
...
void Connection::async_write(const Ptr &conn, uint8_t *buff, size_t size)
{
   ...
   boost::asio::async_write(m_socket, boost::asio::buffer(buff, size),
                            [conn, start, buff](const boost::system::error_code &err, size_t size) {
                               delete[] buff;
...
void Connection::set_compression_threshold(std::size_t threshold) { m_compression_threshold = threshold; }
...
void Connection::set_service_id(boost::uuids::uuid id) { this->m_engine_service_id = id; }

boost::uuids::uuid Connection::service_id() { return this->m_engine_service_id; }

void Connection::push_chunk(int x, int z) { m_chunk_queue.push(minecpp::game::block::ChunkPos(x, z)); }

minecpp::game::block::ChunkPos Connection::pop_chunk() { return m_chunk_queue.pop(); }

bool Connection::has_chunks() { return !m_chunk_queue.empty(); }

tcp::socket &Connection::socket() { return m_socket; }

ConnectionId Connection::id() const { return m_id; }

mb::size Connection::compression_threshold() const { return m_compression_threshold; }

mb::u8 *Connection::alloc_byte() { return m_byte_pool.construct(0); }

void Connection::free_byte(mb::u8 *byte) { m_byte_pool.destroy(byte); }
...
void async_read_packet(const Connection::Ptr &conn, Protocol::Handler &handler)
{
   async_read_varint(conn, 0u, 0u, [conn, &handler](mb::u32 packet_size) {
      ...
      boost::asio::async_read(conn->socket(), *packet_buff,
                              [conn, &handler, packet_buff](const boost::system::error_code &err, size_t size) {
                                 if (err) {
                                    delete packet_buff;
                                    spdlog::debug("error reading data from client: {}", err.message());
