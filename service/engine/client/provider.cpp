...
mb::result<Provider> Provider::create(const Config &cfg)
{
   ...
   try {
      std::transform(cfg.addresses.begin(), cfg.addresses.end(), std::back_inserter(services),
                     [&uuid_to_id, service_count = 0](const std::string &adr) mutable {
...
const std::vector<Provider::Service> &Provider::get_services() const { return m_services; }
