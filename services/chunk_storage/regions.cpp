...
result<std::unique_ptr<RegionFile>> RegionFile::load(const std::string &path) {
   std::fstream file;
   file.open(path);
   if (!file.is_open()) {
...
result<RegionFile &> Regions::get_region(int x, int z) {
...
result<RegionFile &> Regions::load_region(int x, int z) {
...
result<std::vector<uint8_t>> Regions::read_chunk(int x, int z) {
