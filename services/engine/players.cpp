...
void PlayerManager::join_player(Game::World &w, std::string name, uuid id) {
   auto player_data = load_player_data(w, id);

   auto entity_id = entities.spawn(std::forward<Game::Entity::Entity>(player_data.to_entity()));
...
Game::PlayerData PlayerManager::load_player_data(Game::World &w, uuid id) {
   // TODO: cache player data
   std::ifstream f_player_data;
   f_player_data.open(Utils::format("{}/{}.dat", players_path, boost::uuids::to_string(id)));
   if (!f_player_data.is_open()) {
      auto data = Game::PlayerData(id);
...
Utils::Vec3 PlayerManager::get_spawn_position(Game::World &w) {
...
Game::Entity::Entity &PlayerManager::get_entity(uuid id) { return entities.get_entity(get_player(id).get_entity_id()); }
