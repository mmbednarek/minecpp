...
template <typename T>
concept Enumerable = requires(T a, size_t s, std::string_view str,
                              uint32_t index) {
   s = T::num();
   str = a.str();
   index = a.index();
...
template <char... chars> struct string_conv_helper<enum_string<chars...>> {
...
template <typename... Enums> class Enum {
 ...
 public:
   ...
   explicit Enum(const char *name)
       : i(std::distance(strings.begin(),
                         std::find(strings.begin(), strings.end(), name))) {
      if (i >= sizeof...(Enums)) {
         throw std::runtime_error("invalid enum index");
      }
   }

   template <typename Cmp> static constexpr Enum<Enums...> from() {
      return Enum<Enums...>(find_enum<Cmp, Enums...>(0));
   }

   template <typename Cmp> static constexpr uint32_t index_of() {
