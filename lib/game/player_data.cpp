...
void PlayerData::load(nbt::Reader &r, nbt::TagId tagid, const std::string &name)
{
   ...
   case TagId::List:
      ...
      if (name == "EnderItems") {
         r.read_list([this](nbt::Reader &r) { ender_chest.emplace_back(InventoryItem(r)); });
         return;
      }
      if (name == "Inventory") {
         r.read_list([this](nbt::Reader &r) { inventory.emplace_back(InventoryItem(r)); });
         return;
      }
      if (name == "Attributes") {
         r.read_list([this](nbt::Reader &r) {
            double base = 0.0;
            std::string name;
            r.read_compound([&base, &name](nbt::Reader &r, nbt::TagId tagid, const std::string &key) {
   ...
   case TagId::Long:
      ...
      if (name == "UUIDMost") {
         uint64_t value = r.read_long();
         std::memcpy(id.data + sizeof(uint64_t), (char *) &value, sizeof(uint64_t));
         return;
      }
      break;
   default: break;
...
entity::Entity PlayerData::to_entity()
{
   ...
   for (const auto &at : attributes) {
      player_entity.attributes.set_attribute(entity::to_attribute_name(at.first), at.second);
   }
...
{
   r.read_compound([this](nbt::Reader &r, TagId tagid, const std::string &name) {
      switch (tagid) {
      case TagId::Float:
         if (name == "flySpeed") {
