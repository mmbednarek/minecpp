...
Recipe::Recipe(ItemStack item, std::string group, Recipe::CraftingShaped details) :
    _outcome_item(item), _type(RecipeType::CraftingShaped), _details(std::move(details)), _group(std::move(group))
{}

Recipe::Recipe(ItemStack item, std::string group, Recipe::CraftingShapeless details) :
    _outcome_item(item), _type(RecipeType::CraftingShapeless), _details(std::move(details)), _group(std::move(group))
{}

Recipe::Recipe(ItemStack item, RecipeType method) :
    _outcome_item(item), _type(method), _details(Recipe::Special{}), _group("special")
{}

Recipe::Recipe(ItemStack item, std::string group, RecipeType method, Recipe::HeatTreatment smelted) :
    _outcome_item(item), _type(method), _details(std::move(smelted)), _group(std::move(group))
{
   switch (method) {
   case Smelting:
   case Blasting:
...
Recipe::Recipe(ItemStack item, std::string group, Recipe::StoneCutting details) :
    _outcome_item(item), _type(RecipeType::StoneCutting), _details(std::move(details)), _group(std::move(group))
{}
